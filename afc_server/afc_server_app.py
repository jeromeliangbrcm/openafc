...
async def available_spectrum_inquiry_internal(
    ...
    # Internal route therefore requires the narrower dispatcher token
    # outright. Rejecting (not downgrading) on failure: falling back to
    # external-caller semantics would serve the request WITHOUT the
    # dispatcher-token/mTLS enforcement that available_spectrum_inquiry
    # applies to every external request.
    expected_disp = _get_dispatcher_token()
    if not expected_disp or not hmac.compare_digest(
            x_afc_dispatcher_token or "", expected_disp):
        raise fastapi.HTTPException(
